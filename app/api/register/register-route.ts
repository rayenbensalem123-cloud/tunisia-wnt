...
export async function POST(req: Request) {
  try {
    const { firstName, lastName, username, password, memberType } = await req.json()

    if (!firstName?.trim() || !lastName?.trim() || !username?.trim() || !password) {
      return NextResponse.json({ error: 'Fill all fields' }, { status: 400 })
    }
    if (memberType !== 'player' && memberType !== 'staff') {
      return NextResponse.json({ error: 'Select whether you are a player or staff' }, { status: 400 })
    }
    ...
    const { error: profileErr } = await supabaseAdmin.from('profiles').insert({
      ...
      permissions: {
        ...
        viewMedical: false, editMedical: false,
      },
    })
    if (profileErr) {
